...
void cooling_radius_momentum_feedback(int target, int mode, int thread_id, int numnodes, int *firstnode, data_in *in, data_out *out)
{
  ...
  terminal_mom *= pow(All.FeedbackEfficiency * (n_SNII + n_SNIa), 13. / 14.) * All.cf_atime;      /* comoving code units  */
  ...
  double rcool = 28.4 * PARSEC * facZ * pow(n0, -3. / 7.) * pow(All.FeedbackEfficiency * (n_SNII + n_SNIa), 2. / 7.) /
                 All.UnitLength_in_cm * All.HubbleParam / All.cf_atime;
...
#ifdef SMUGGLE_FACE_AREA_BALANCE
  MyDouble momentum_norm = in->TotFaceNorm;
  if(momentum_norm != 0.0)
    momentum_norm = 1.0 / momentum_norm;

  MyDouble energy_norm = in->TotEnergyNorm;
  if(energy_norm != 0.0)
    energy_norm = (2.0 * TotalEnergyReleased * TotalMassReleased) /
                  (TotalMomentumReleased * TotalMomentumReleased * energy_norm * momentum_norm * momentum_norm);
#endif 
          ...
          if(r2 < h2)
            {
              ...
              if(TotalEnergyReleased > 0)
                {
                  if(r < rlim)
                    {
                      // compute all in reference frame of star, then convert to lab frame
#ifdef SMUGGLE_FACE_AREA_BALANCE
		      ...
		      if(dr[2] > 0)
                      ...
                      dpSNv[0] = -1.0 * momentum_norm * TotalMomentumReleased  * x_weight_fac; /* dp of a single cell in comoving units */
                      dpSNv[1] = -1.0 * momentum_norm * TotalMomentumReleased  * y_weight_fac; /* dp of a single cell in comoving units */
                      dpSNv[2] = -1.0 * momentum_norm * TotalMomentumReleased  * z_weight_fac; /* dp of a single cell in comoving units */
                      ...
                      dpSN = sqrt(dpSN);
                      de   = energy_norm * 0.5 * dpSN * dpSN / (TotalMassReleased * mweight_fac);
                  ...
                  if(r < rlim)
                    {
                      ...
                      dpAGB     = dpAGBv[0] * dpAGBv[0] + dpAGBv[1] * dpAGBv[1] + dpAGBv[2] * dpAGBv[2];

                      double msq   = momc[0] * momc[0] + momc[1] * momc[1] + momc[2] * momc[2];
                      double cdot  = momc[0] * dpAGBv[0] + momc[1] * dpAGBv[1] + momc[2] * dpAGBv[2];
                      double massr = P[j].Mass / (mweight_fac * TotalMassReleasedAGB);

                      boost_fac = fmin(sqrt(1. + massr), terminal_mom_AGB / TotalMomentumReleasedAGB);

                      double sol, boost_fac_max;
                      if(cdot > 0.0)
                        {
                          sol           = cdot / dpAGB + sqrt(1. + massr + msq / (dpAGB * massr) + cdot * cdot / (dpAGB * dpAGB));
                          boost_fac_max = (1. + massr + msq / (dpAGB * massr)) / sol;
                        }
                      else
                        boost_fac_max = -cdot / dpAGB + sqrt(1. + massr + msq / (dpAGB * massr) + cdot * cdot / (dpAGB * dpAGB));
