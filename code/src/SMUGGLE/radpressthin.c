 ...
 * 
 * \file        src/SMUGGLE/radpressthin.c
 * \date        03/2020
...
void radpressthin(void)
{
  ...
  All.BunchSize =
      (int)((All.BufferSize * 1024 * 1024) /
            (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct radpressthindata_in) +
             sizeof(struct radpressthindata_out) + smax(sizeof(struct radpressthindata_in), sizeof(struct radpressthindata_out))));
  DataIndexTable = (data_index *)mymalloc("DataIndexTable", All.BunchSize * sizeof(data_index));
  DataNodeList   = (struct data_nodelist *)mymalloc("DataNodeList", All.BunchSize * sizeof(struct data_nodelist));

  /* Create list of targets. We do this here to simplify the treatment of the
   * two possible sources of points */
  ...
  do
    {
      ...
      RadpressthinDataGet =
          (struct radpressthindata_in *)mymalloc("RadpressthinDataGet", nimport * sizeof(struct radpressthindata_in));
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              /* get the particles */
              MPI_Sendrecv(&RadpressthinDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct radpressthindata_in),
                           MPI_BYTE, recvTask, TAG_HYDRO_A, &RadpressthinDataGet[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct radpressthindata_in), MPI_BYTE, recvTask, TAG_HYDRO_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }

      myfree(RadpressthinDataIn);
      RadpressthinDataResult =
          (struct radpressthindata_out *)mymalloc("RadpressthinDataResult", nimport * sizeof(struct radpressthindata_out));
      RadpressthinDataOut =
          (struct radpressthindata_out *)mymalloc("RadpressthinDataOut", nexport * sizeof(struct radpressthindata_out));
      ...
      for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
          ...
          if(recvTask < NTask)
            if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
              MPI_Sendrecv(&RadpressthinDataResult[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct radpressthindata_out),
                           MPI_BYTE, recvTask, TAG_HYDRO_B, &RadpressthinDataOut[Send_offset[recvTask]],
                           Send_count[recvTask] * sizeof(struct radpressthindata_out), MPI_BYTE, recvTask, TAG_HYDRO_B, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
        }
  ...
  struct radpressthinresultsimported_data *tmp_results =
      mymalloc("tmp_results", nexport * sizeof(struct radpressthinresultsimported_data));
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&RadpressthinResultsImported[Recv_offset[recvTask]],
                       ...
                       &tmp_results[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct radpressthinresultsimported_data),
...
/* [All.IonizingLumPerSolarMass] = photons s^-1 Msun^-1; */
#ifdef SMUGGLE_RADPRESS_OPT_THIN_LUMPERMASS
  lum_per_mass =
      (All.UnitMass_in_g / SOLAR_MASS / All.HubbleParam * All.UnitTime_in_s / All.HubbleParam) * All.IonizingLumPerSolarMass;
#else
  ...
  /* [All.IonizingLumPerSolarMass] = erg s^-1 Msun^-1; [lum_per_mass] =
   * UnitEnergy UnitTime^-1 UnitMass^-1 */
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      /* dt = (All.otvet_Radiation_Ti_endstep - All.otvet_Radiation_Ti_begstep) *
      ...
      nH =
          HYDROGEN_MASSFRAC * SphP[i].Density / PROTONMASS * All.UnitMass_in_g / All.HubbleParam; /* LVS shall I multiply by a3inv?? */
      nHI = SphP[i].HI * nH;
      ...
      for(k = 0; k < 3; k++)
        {
          ...
          if(!gsl_finite(dvel))
            terminate(
                "SMUGGLE_RADPRESS_OPT_THIN: bad radiation pressure dvel=%g "
                "P[i].Mass=%g SphP[i].RadPress[k]=%g dtime=%g ID=%d",
                dvel, P[i].Mass, SphP[i].RadPress[k], dtime, P[i].ID);
        }
  ...
  mpi_printf("SMUGGLE_RADPRESS_OPT_THIN: kin. energy ratio = (%g - %g) \n", global_ekin_min, global_ekin_max);
}
      ...
      while(no >= 0)
        {
          if(no < Tree_MaxPart) /* single particle */
...
#ifdef GFM
                  age_in_Gyr = get_time_difference_in_Gyr(STP(no).BirthTime, All.Time);
#else
                  age_in_Gyr = 0.;
#endif
          ...
          else if(no < Tree_MaxPart + Tree_MaxNodes) /* internal node */
            {
              if(mode == 1)
                {
                  if(no < Tree_FirstNonTopLevelNode) /* we reached a top-level node again,
                                                        which means that we are done with
                                                        the branch */
                    {
                      no = -1;
                      continue;
              ...
              if(mode == 0)
                {
                  ...
                  if(Exportnodecount[task] == NODELISTLENGTH)
                    {
                      if(*nexport >= All.BunchSize)
                        {
                          /* out of buffer space. Need to discard work for this particle and
                           * interrupt */
                          *nexport = nexport_save;
                          if(nexport_save == 0)
                            terminate("17998"); /* in this case, the buffer is too small to
                                                   process even a single particle */
                  ...
                  DataNodeList[Exportindex[task]].NodeList[Exportnodecount[task]++] =
                      DomainNodeIndex[no - (Tree_MaxPart + Tree_MaxNodes)];
 ...
 */
#ifndef SMUGGLE_RADPRESS_OPT_THIN_LUMPERMASS
