...
void begrun1(void)
{
  ...
  //JONAH EDIT - if RESTART_RESTART create generator from restart state
  /*
  if (RestartFlag == RESTART_RESTART){
      File *rng_stream = fopen("%s/restartfiles/rng_state.%d"%(All.OutputDir, ThisTask),'r');
      gsl_rng_fread(rng_stream, random_generator);
      fclose(rng_stream);

      File *rng_stream_aux = fopen("%s/restartfiles/rng_state_aux.%d"%(All.OutputDir, ThisTask),'r');
      gsl_rng_fread(rng_stream_aux, random_generator_aux);
      fclose(rng_stream_aux);
  } else {
  */
