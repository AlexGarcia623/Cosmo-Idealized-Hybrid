      ...
      if(!((modus == MODUS_WRITE && write_success[ThisTask]) || (modus == MODUS_CHECK && write_success[get_file_to_check(ThisTask)])))
        {
          if(modus == MODUS_CHECK)
          ...
          else
            {
              ...
              //JONAH EDIT - write file here?
              //File *rng_stream = fopen("%s/restartfiles/rng_state.%d"%(All.OutputDir, ThisTask),'w');
              //gsl_rng_fwrite(rng_stream, random_generator);
              //fclose(rng_stream);

              //File *rng_stream_aux = fopen("%s/restartfiles/rng_state_aux.%d"%(All.OutputDir, ThisTask),'w');
              //gsl_rng_fwrite(rng_stream_aux, random_generator_aux);
