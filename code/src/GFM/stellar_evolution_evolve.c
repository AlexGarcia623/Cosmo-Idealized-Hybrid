                 ...
                 stellar_evolution_data *sed)
{
  ...
  sed->number_of_SNII = sed->SNII_events_in_timestep;
#endif
  ...
  if(aux_SNII > 0.0)
    ...
    {
      //no SN exploded, we can stop here
      sed->number_of_SNII = 0.0;
      return;
    }
                     ...
                     MyFloat log_metallicity, MyFloat *initial_metals, stellar_evolution_data *sed)
{
   ...
   * masses range are capped at max mass in the IMF, but for OB stars
   * enrichment has to start immediately and not when 100 M_sun stars
   * begin to evolve
   */
  if(log_min_mass < log10(All.IMF_MaxMass_Msun))
    if(log_min_mass >= log_max_mass)
      {
...
{
  MyFloat dt;
  MyFloat SNII_rate = 3.1e-4; //see FIRE-2 (this has been averaged over the 2 intervals)

  if((age_of_star_in_Gyr + dtime_in_Gyr) < 3.401e-3 || age_of_star_in_Gyr > 0.03753)
    return 0.0;
  else if(age_of_star_in_Gyr <= 3.401e-3)
    dt = dtime_in_Gyr - (3.401e-3 - age_of_star_in_Gyr);
  else
    dt = dmin(dtime_in_Gyr, 0.03753 - age_of_star_in_Gyr);

  return 1e3 * SNII_rate * dt;
}
#endif
...
#elif defined(GFM_CONST_IMF) && (GFM_CONST_IMF == 2)
      /* Kroupa 2001 */
      if(mass < 0.08)
        imf_by_number[i] =  pow(mass, -0.3) / 0.178209;
      else if((mass >= 0.08) && (mass < 0.5))
        imf_by_number[i] = 0.08 / 0.178209 * pow(mass, -1.3);
      else
        imf_by_number[i] = 0.04 / 0.178209 * pow(mass, -2.3);
...
void test_stellar_evolution(void)
{
  ...
  MyFloat age_of_star_max = 10.0;
  MyFloat age_of_star, age_of_star_in_Gyr = 0.0;
  MyFloat initial_mass_in_Msun;
  ...
  /* compute the initial stellar mass in solar masses */
  initial_mass_in_Msun = STP(iPart).InitialMass * All.UnitMass_in_g / (All.HubbleParam * SOLAR_MASS);
  ...
  /*
     ...
     -solar = 0.0127
   */
  STP(iPart).Metallicity = 0.05 * GFM_SOLAR_METALLICITY;  // 0.0127 for diff test
  ...
  /* convert times in Gyr to internal code units */
  dtime *= SEC_PER_GIGAYEAR * All.HubbleParam / All.UnitTime_in_s;
  dtime_output *= SEC_PER_GIGAYEAR * All.HubbleParam / All.UnitTime_in_s;
  age_of_star_max *= SEC_PER_GIGAYEAR * All.HubbleParam / All.UnitTime_in_s;

  for(age_of_star = 0.0; age_of_star <= age_of_star_max; age_of_star += dtime)
    {
      age_of_star_in_Gyr = get_time_difference_in_Gyr(STP(iPart).BirthTime, STP(iPart).BirthTime + age_of_star);
      ...
      if(age_of_star >= next_outputtime)
        {
          ...
          /* remaining stellar mass fraction and released mass per enrichment channel */
          fprintf(fd[6], "%10.5e %10.5e %10.5e %10.5e %10.5e %10.5e %10.5e\n", age_of_star_in_Gyr, 
			  P[iPart].Mass / STP(iPart).InitialMass, sed.total_mass_released, sed.AGB_mass_released, 
			  sed.SNIa_mass_released, sed.SNII_mass_released, sed.OB_mass_released);
  ...
  mpi_printf("NSNIatot=%g, expected=%g\n", NtotSNIa / initial_mass_in_Msun, All.SNIa_Rate_Norm);
  mpi_printf("NSNIItot=%g, SNII/SNIa=%g\n", NtotSNII / initial_mass_in_Msun, NtotSNII / NtotSNIa);
  mpi_printf("ejected mass fraction: %g\n", (STP(iPart).InitialMass - P[iPart].Mass) / STP(iPart).InitialMass);
  mpi_printf("stellar age in Gyr: %g\n", age_of_star_in_Gyr);
