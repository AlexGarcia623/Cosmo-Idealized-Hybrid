...
int read_yield_tables(void)
{
  ...
  char fname[MAXLEN_PATH + 30], setname[MAXLEN_PATH];
  ...
  file_path_sprintf(fname, "%s/%s", All.YieldTablePath, GFM_SNIa_YIELDNAME);
  ...
  file_path_sprintf(fname, "%s/%s", All.YieldTablePath, GFM_SNII_YIELDNAME);
  ...
  file_path_sprintf(fname, "%s/%s", All.YieldTablePath, GFM_AGB_YIELDNAME);
  ...
  file_path_sprintf(fname, "%s/%s", All.YieldTablePath, GFM_LIFETIMENAME);
  ...
  for(i = 0; i < yieldsSNII.N_Z; i++)
    {
      file_path_sprintf(setname, "/Yields/%s/Yield", tempname1[i]);
      ...
      file_path_sprintf(setname, "/Yields/%s/Ejected_mass", tempname1[i]);
      dataset = my_H5Dopen(file_id, setname);
      my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, tempej1, setname);
      my_H5Dclose(dataset, setname);
      file_path_sprintf(setname, "/Yields/%s/Total_Metals", tempname1[i]);
      dataset = my_H5Dopen(file_id, setname);
  ...
  for(i = 0; i < yieldsAGB.N_Z; i++)
    {
      file_path_sprintf(setname, "/Yields/%s/Yield", tempname2[i]);
      dataset = my_H5Dopen(file_id, setname);
      my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, tempyield2, setname);
      my_H5Dclose(dataset, setname);
      file_path_sprintf(setname, "/Yields/%s/Ejected_mass", tempname2[i]);
      dataset = my_H5Dopen(file_id, setname);
      my_H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, tempej2, setname);
      my_H5Dclose(dataset, setname);
      file_path_sprintf(setname, "/Yields/%s/Total_Metals", tempname2[i]);
      dataset = my_H5Dopen(file_id, setname);
...
void spline_interpolate_yields(int spline_element_index)
{
  ...
  file_path_sprintf(element_name, "%s", ElementNames[spline_element_index]); /* metals, including Helium */
  ...
  if(element_index < 0)
    {
      mpi_printf("GFM_STELLAR_EVOLUTION: SNIa: element not found %s\n", element_name);
      yieldsSNIa.spline[spline_element_index] = 0;
  ...
  if(element_index < 0)
    {
      mpi_printf("GFM_STELLAR_EVOLUTION: SNII: element not found %s\n", element_name);

      for(i = 0; i < yieldsSNII.N_Z; i++)
        {
          for(k = 0; k < GFM_N_MASS_BINS; k++)
            {
              yieldsSNII.spline[i][spline_element_index][k] = 0;
            }
        }
    }
  else
...
void spline_interpolate_ejecta(void)
{
  int i, k;
      ...
      for(k = 0; k < GFM_N_MASS_BINS; k++)
        {
          ...
          if(element_index("Hydrogen") > 0)
            total_mass += yieldsSNII.spline[i][element_index("Hydrogen")][k] +
                          (1.0 - yieldsSNII.Metallicity[i]) * 0.75 * yieldsSNII.Ejecta_spline[i][k];
          if(element_index("Helium") > 0)
            total_mass += yieldsSNII.spline[i][element_index("Helium")][k] +
                          (1.0 - yieldsSNII.Metallicity[i]) * 0.25 * yieldsSNII.Ejecta_spline[i][k];
