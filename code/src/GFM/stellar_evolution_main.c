...
#define WARN_TOLERANCE 0.01 // 0.00001 JONAH EDIT
...
void do_stellar_evolution(MyFloat age_of_star_in_Gyr, MyFloat dtime_in_Gyr, int iPart, stellar_evolution_data* sed)
{
  ...
  /* If OB/AGB winds are defined, then they should start immediately after the creation of 
   * the star particle, not after stars of log_max_mass have evolved. If they are not, 
   * then this part of stellar evolution can be skipped */
#ifndef SMUGGLE_AGB_WINDS
      if(log_min_mass == log_max_mass)
        return;
#endif
      if (dtime_in_Gyr == 0) //JONAH EDIT
          return;
...
#error "GFM_VARIABLE_IMF mode is not ok"
      ...
      if (STP(iPart).SNTime < STP(iPart).BirthTime) //JONAH EDIT
        STP(iPart).SNTime = All.Time - dtime_in_Gyr;
      ...
      sed->SNII_events_in_timestep = get_expected_SNII_events(age_SN_in_Gyr, dtime_SN_in_Gyr);
#endif
...
#ifdef SMUGGLE_CONSTANT_SNII_RATE
      sed->SNII_events_in_timestep = get_expected_SNII_events(age_of_star_in_Gyr, dtime_in_Gyr);
#endif
...
#ifdef SMUGGLE_AGB_WINDS
      //Ages are initialized to >0, BirthTime is initialized to 0
      if (STP(iPart).OBTime < STP(iPart).BirthTime) //JONAH EDIT
        STP(iPart).OBTime = All.Time - dtime_in_Gyr;
      ...
      //printf("AgeS: %g, AOB: %g, dt: %g, lmam: %g, lmim: %g, lm: %g", age_of_star_in_Gyr, age_OB_in_Gyr, dtime_in_Gyr, log_max_mass, log_min_mass, log_metallicity);
  ...
  for(i = 0; i < Nstar; i++)
    {
      ...
      StarParticle[i].AGBWindSpeed         = sqrt(2e12 * psi * All.WindSpeedFactor) / All.UnitVelocity_in_cm_per_s;
