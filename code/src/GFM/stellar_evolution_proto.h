...
#ifdef SMUGGLE_CONSTANT_SNII_RATE
MyFloat get_expected_SNII_events(MyFloat age_of_star_in_Gyr, MyFloat dtime_in_Gyr);
#endif
...
/* interpolation */
static inline MyFloat interpol_1d(MyFloat *table, int i, double dx);
...
#ifndef RADCOOL
static inline MyFloat interpol_5d(MyFloat *****table, int i, int j, int k, int l, int m, double dx, double dy, double dz, double dw,
                                  double dv);
#endif

#ifdef RADCOOL
static inline MyFloat interpol_5d(float *****table, int i, int j, int k, int l, int m, double dx, double dy, double dz, double dw,
                                  double dv);
#ifdef RADCOOL_HOTHALO
static inline MyFloat interpol_8d(float ********table, int ir, int io, int in, int i6, int i7, int i8, int id, int it, double dir,
                                  double dio, double din, double di6, double di7, double di8, double did, double dit);
#endif
#endif

static inline MyFloat interpol_1d(MyFloat *table, int i, double dx) { return (1 - dx) * table[i] + dx * table[i + 1]; }
...
static inline MyFloat interpol_3d(MyFloat ***table, int i, int j, int k, double dx, double dy, double dz)
{
  ...
  return dxl * dyl * dzl * table[il][jl][kl] + dxl * dyl * dzr * table[il][jl][kr] + dxl * dyr * dzl * table[il][jr][kl] +
         dxl * dyr * dzr * table[il][jr][kr] + dxr * dyl * dzl * table[ir][jl][kl] + dxr * dyl * dzr * table[ir][jl][kr] +
         dxr * dyr * dzl * table[ir][jr][kl] + dxr * dyr * dzr * table[ir][jr][kr];
}
...
#ifndef RADCOOL
inline MyFloat interpol_5d(MyFloat *****table, int i, int j, int k, int l, int m, double dx, double dy, double dz, double dw,
                           double dv)
#else
...
#endif
{
  ...
  return dxl * dyl * dzl * dwl * dvl * table[il][jl][kl][ll][ml] + dxl * dyl * dzl * dwl * dvr * table[il][jl][kl][ll][mr] +
         dxl * dyl * dzl * dwr * dvl * table[il][jl][kl][lr][ml] + dxl * dyl * dzl * dwr * dvr * table[il][jl][kl][lr][mr] +
         dxl * dyl * dzr * dwl * dvl * table[il][jl][kr][ll][ml] + dxl * dyl * dzr * dwl * dvr * table[il][jl][kr][ll][mr] +
         dxl * dyl * dzr * dwr * dvl * table[il][jl][kr][lr][ml] + dxl * dyl * dzr * dwr * dvr * table[il][jl][kr][lr][mr] +
         dxl * dyr * dzl * dwl * dvl * table[il][jr][kl][ll][ml] + dxl * dyr * dzl * dwl * dvr * table[il][jr][kl][ll][mr] +
         dxl * dyr * dzl * dwr * dvl * table[il][jr][kl][lr][ml] + dxl * dyr * dzl * dwr * dvr * table[il][jr][kl][lr][mr] +
         dxl * dyr * dzr * dwl * dvl * table[il][jr][kr][ll][ml] + dxl * dyr * dzr * dwl * dvr * table[il][jr][kr][ll][mr] +
         dxl * dyr * dzr * dwr * dvl * table[il][jr][kr][lr][ml] + dxl * dyr * dzr * dwr * dvr * table[il][jr][kr][lr][mr] +
         dxr * dyl * dzl * dwl * dvl * table[ir][jl][kl][ll][ml] + dxr * dyl * dzl * dwl * dvr * table[ir][jl][kl][ll][mr] +
         dxr * dyl * dzl * dwr * dvl * table[ir][jl][kl][lr][ml] + dxr * dyl * dzl * dwr * dvr * table[ir][jl][kl][lr][mr] +
         dxr * dyl * dzr * dwl * dvl * table[ir][jl][kr][ll][ml] + dxr * dyl * dzr * dwl * dvr * table[ir][jl][kr][ll][mr] +
         dxr * dyl * dzr * dwr * dvl * table[ir][jl][kr][lr][ml] + dxr * dyl * dzr * dwr * dvr * table[ir][jl][kr][lr][mr] +
         dxr * dyr * dzl * dwl * dvl * table[ir][jr][kl][ll][ml] + dxr * dyr * dzl * dwl * dvr * table[ir][jr][kl][ll][mr] +
         dxr * dyr * dzl * dwr * dvl * table[ir][jr][kl][lr][ml] + dxr * dyr * dzl * dwr * dvr * table[ir][jr][kl][lr][mr] +
         dxr * dyr * dzr * dwl * dvl * table[ir][jr][kr][ll][ml] + dxr * dyr * dzr * dwl * dvr * table[ir][jr][kr][ll][mr] +
         dxr * dyr * dzr * dwr * dvl * table[ir][jr][kr][lr][ml] + dxr * dyr * dzr * dwr * dvr * table[ir][jr][kr][lr][mr];
}

#ifdef RADCOOL_HOTHALO
inline MyFloat interpol_8d(float ********table, int ir, int io, int in, int i6, int i7, int i8, int id, int it, double dir, double dio,
                           double din, double di6, double di7, double di8, double did, double dit)
{
  int idx[8][2];
  double dval[8][2];
        ...
        for(cn3 = 0; cn3 < 2; cn3++)
          for(cn4 = 0; cn4 < 2; cn4++)
            for(cn5 = 0; cn5 < 2; cn5++)
              for(cn6 = 0; cn6 < 2; cn6++)
                for(cn7 = 0; cn7 < 2; cn7++)
                  sum += dval[0][cn0] * dval[1][cn1] * dval[2][cn2] * dval[3][cn3] * dval[4][cn4] * dval[5][cn5] * dval[6][cn6] *
                         dval[7][cn7] *
