  ...
  for(idx = 0; idx < TimeBinsGravity.NActiveParticles; idx++)
    {
      ...
      if (All.Time == All.TimeBegin) //JONAH EDIT
          break;
      ...
      if(P[i].Type == 4 && P[i].Mass > 0 && STP(i).BirthTime > 0)
#endif
        {
          ...
          StarParticle[Nstar].RadFeed_NumNgbCold = 0;
          StarParticle[Nstar].PhotoionizationEvents = 0;
          ...
          for(k = 0; k < 7; k++)
            StarParticle[Nstar].FaceAreas[k] = 0;

          StarParticle[Nstar].TotFaceNorm = 0;
          StarParticle[Nstar].TotEnergyNorm = 0;
#endif
          Nstar++;
        }
    }
