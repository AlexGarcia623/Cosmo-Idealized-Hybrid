...
int allocate_stellar_photometrics_tables(void)
{
  ...
  stellarLuminosityTable.LogMetallicity_bins =
      (MyFloat *)mymalloc("stellarLuminosityTable.LogMetallicity_bins", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat));

  stellarLuminosityTable.LogAgeInGyr_bins =
      (MyFloat *)mymalloc("stellarLuminosityTable.LogAgeInGyr_bins", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat));

  stellarLuminosityTable.Magnitude_U =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_U", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_B =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_B", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_V =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_V", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_K =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_K", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_g =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_g", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_r =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_r", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_i =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_i", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));
  stellarLuminosityTable.Magnitude_z =
      (MyFloat **)mymalloc("stellarLuminosityTable.Magnitude_z", stellarLuminosityTable.N_LogMetallicity * sizeof(MyFloat **));

  for(i = 0; i < stellarLuminosityTable.N_LogMetallicity; i++)
    {
      stellarLuminosityTable.Magnitude_U[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_U", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_B[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_B", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_V[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_V", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_K[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_K", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_g[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_g", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_r[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_r", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_i[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_i", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
      stellarLuminosityTable.Magnitude_z[i] =
          (MyFloat *)mymalloc("stellarLuminosityTable.Magnitude_z", stellarLuminosityTable.N_LogAgeInGyr * sizeof(MyFloat *));
    }

  return 0;
}

int read_stellar_photometrics_tables(void)
{
  int i, k, l;
  char fname[MAXLEN_PATH + 30];
  ...
  file_path_sprintf(fname, "%s/%s", All.PhotometricsTablePath, "stellar_photometrics.hdf5");
  ...
  tmpdbl =
      (double *)mymalloc("tmpdbl", stellarLuminosityTable.N_LogMetallicity * stellarLuminosityTable.N_LogAgeInGyr * sizeof(double));
  ...
  if(ThisTask == 0)
    {
      ...
      file_path_sprintf(buf, "%s/stellar_photometrics_bins.txt", All.OutputDir);
      FILE *fd = fopen(buf, "w");
...
void get_stellar_photometrics(MyFloat ageInGyr, MyFloat metallicity, MyFloat mass, stellar_photometrics *st_photo)
{
  ...
  if(log_AgeInGyr > stellarLuminosityTable.LogAgeInGyr_bins[0])
    {
      for(i1 = 0; i1 < stellarLuminosityTable.N_LogAgeInGyr - 2 && log_AgeInGyr > stellarLuminosityTable.LogAgeInGyr_bins[i1 + 1];
          i1++)
        ;
      ...
      if(log_AgeInGyr >= stellarLuminosityTable.LogAgeInGyr_bins[0] &&
         log_AgeInGyr <= stellarLuminosityTable.LogAgeInGyr_bins[stellarLuminosityTable.N_LogAgeInGyr - 1])
  ...
  if(log_Metallicity > stellarLuminosityTable.LogMetallicity_bins[0])
    {
      for(k1 = 0;
          k1 < stellarLuminosityTable.N_LogMetallicity - 2 && log_Metallicity > stellarLuminosityTable.LogMetallicity_bins[k1 + 1];
          k1++)
        ;
      ...
      if(log_Metallicity >= stellarLuminosityTable.LogMetallicity_bins[0] &&
         log_Metallicity <= stellarLuminosityTable.LogMetallicity_bins[stellarLuminosityTable.N_LogMetallicity - 1])
...
void test_stellar_photometrics(double mass_in_solar, double metallicity, double log_AgeInGyr_min, double log_AgeInGyr_max,
                               int log_AgeInGyr_bins)
{
  ...
  fprintf(fd,
          "#log_AgeInGyr   Magnitude_U   Magnitude_B   Magnitude_V   Magnitude_K   Magnitude_g   Magnitude_r   Magnitude_i   "
          "Magnitude_z\n");
    ...
    for(int i = 0; i < GFM_STELLAR_PHOTOMETRICS_DIRECTIONS; i++)
      {
        ...
        // StellarPhotometricsRandomAngles[i][0], StellarPhotometricsRandomAngles[i][1]);
      }
