  ...
  for(i = 0; i < npart; i++)
    {
      ...
      if(StarParticle[i].GasColumnDensity <= 0.0)
        terminate("This should not happen: i=%d drho/dr = %g\n", i, StarParticle[i].GasColumnDensity);      
...
//            {
              ...
              loc_ism_dens   += P[j].Mass * wk;
              loc_ism_densH  += SphP[j].MetalsFraction[element_index("Hydrogen")] * P[j].Mass * wk;
              loc_ism_z_dens += P[j].Mass * wk * SphP[j].Metallicity;

              grad_rho[0] += dwk * P[j].Mass * dr[0] / r; 
              grad_rho[1] += dwk * P[j].Mass * dr[1] / r; 
              grad_rho[2] += dwk * P[j].Mass * dr[2] / r; 

              //s_lenght += (-dwk * P[j].Mass);
...
#endif
#ifdef SMUGGLE_RADIATION_FEEDBACK
  s_lenght = sqrt(grad_rho[0]*grad_rho[0] + grad_rho[1]*grad_rho[1] + grad_rho[2]*grad_rho[2]);
